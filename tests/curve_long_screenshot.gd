extends Node2D
## Verify SUPER long curves render as a smooth continuous ribbon after the
## decimation budget raise (700 -> 4000 pts). Emulates the editor pipeline:
...
func _editor_place(raw: PackedVector2Array, bid: int) -> void:
	...
	if pts.size() > 4000:
		var keep_step: int = int(ceil(float(pts.size()) / 4000.0))
...
func _ready() -> void:
	...
	get_viewport().get_texture().get_image().save_png("user://curve_long.png")
