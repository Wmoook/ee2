			...
			if _curve_mode and _curve_points.size() >= 2:
				...
				if _curve_points.size() >= 2:
					...
					if spline_pts.size() >= 2:
						...
						# Add solid end cap collision polylines (perpendicular walls)
						var cap_sd: Vector2 = (spline_pts[1] - spline_pts[0]).normalized()
						var cap_sp: Vector2 = Vector2(-cap_sd.y, cap_sd.x)
						var cap_start: PackedVector2Array = PackedVector2Array()
						cap_start.append(spline_pts[0] + cap_sp * 10.0)
						cap_start.append(spline_pts[0] - cap_sp * 10.0)
						WorldManager.add_polyline(cap_start, "both", GameState.selected_block_id)
						var cap_ed: Vector2 = (spline_pts[-1] - spline_pts[-2]).normalized()
						var cap_ep: Vector2 = Vector2(-cap_ed.y, cap_ed.x)
						var cap_end: PackedVector2Array = PackedVector2Array()
						cap_end.append(spline_pts[-1] + cap_ep * 10.0)
						cap_end.append(spline_pts[-1] - cap_ep * 10.0)
						WorldManager.add_polyline(cap_end, "both", GameState.selected_block_id)
				_curve_points.clear()
